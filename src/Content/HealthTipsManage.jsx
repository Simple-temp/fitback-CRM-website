    ...
    const fetchExcercise = async () => {
      try {
        const response = await axios.get(
          `https://qwikit1.pythonanywhere.com/healthTipsVideos`
        );
    ...
    const handleUpdateSubmit = async () => {
      ...
      try {
        const response = await axios.put(
          `https://qwikit1.pythonanywhere.com/healthTipsVideos/${currentExercise.id}`,
          currentExercise
        );
    ...
    return (
      <div>
                ...
                <TableBody>
                  {getExcercise.map((item) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell>{item.healthtips_name || "N/A"}</TableCell>
        ...
        <Dialog open={updateOpen} onClose={handleCloseUpdate} maxWidth="sm">
          <DialogTitle>Update Health Tips</DialogTitle>
          <DialogContent>
            <TextField
              fullWidth
              margin="normal"
              label="Exercise Name"
              value={currentExercise?.healthtips_name || ""}
              onChange={(e) => handleChange("healthtips_name", e.target.value)}
            />
            <TextField
              fullWidth
              margin="normal"
