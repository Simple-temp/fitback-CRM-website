...
const CreateReviewVideo = () => {
  ...
  const createExercise = async () => {
    try {
      ...
      formData.append("user_name", exerciseName);
      if (video1) {
        formData.append("review_video", video1);
      } else {
        formData.append("review_video", "");
      }
      formData.append("youtubelink", youtubeLink);
      formData.append("review_details", review_details);

      const response = await axios.post(
        "https://qwikit1.pythonanywhere.com/userReviewVideo/new",
        formData,
        {
          headers: {
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Create review Video</h1>
      <ToastContainer
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setreview_details(e.target.value)}
            className="border rounded p-2 w-full"
            required
          />
        </div>
...
export default CreateReviewVideo;
